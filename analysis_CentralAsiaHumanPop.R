...
priors=colSums(donnees[bool,14:214],na.rm=TRUE)
abs=seq(1,200,1)
plot(abs[1:100],priors[1:100],ylim=c(0,1),xlim=c(0,200),ylab="Pr(Ha | Sobs)",xlab="simulated sample")
points(abs[101:200],priors[101:200],col='red')
...
mean(priors[1:100])
median(priors[1:100])
lines(c(0,100),c(median(priors[1:100]),median(priors[1:100])))
text(x=45,y=0.05,"median prob = 0.4335",cex=0.75)

mean(priors[101:200])
median(priors[101:200])
lines(c(100,200),c(median(priors[101:200]),median(priors[101:200])),col="red")
...
for(i in 1:101)
{
  type1[i]=sum(priors[1:100]>alpha[i])/100
  type2[i]=sum(priors[101:200]<=alpha[i])/100
}
...
(median(priors[1:100])+median(priors[101:200]))/2
median(priors[1:200])
...
lines(c((median(priors[1:100])+median(priors[101:200]))/2,(median(priors[1:100])+median(priors[101:200]))/2),c(0,1),lty=3)
...
sum(priors[1:100]>priors[201])/100
...
o=order(ecartsb)
priorsdiff=priors[101:200]
pourcentagetype2=cumsum(priorsdiff[o]<=0.29)/100
